...
The **Skill Assessment** flow is built (`modules/skill_assessment`, `skill-assessments` endpoints, `SkillAssessmentRepository`): a hub listing assessable skills and past attempts, and a quiz screen that answers questions one at a time (progress bar, choice chips or a free-text field per `type`) then, after submit, becomes a scored review with per-question correctness. Submit posts every answer at once as `{ "<questionId>": {"value": ...} }`; correctness and the answer key are withheld by the server until submission. Reached from the AI Karier hub via `AiFeature.route`.

The **AI Interview Practice** flow is built (`modules/ai_interview`, `ai-interviews` endpoints, `AiInterviewRepository`): a hub that starts a text-mode practice run or reopens a past one, and a run screen with three phases switched by `InterviewPhase` — answer questions one at a time (each answer posts immediately), an "analysing" wait with a manual refresh (the finalise call may leave the session `analyzing` before the AI verdict lands), then a result with the overall score, summary, strengths, weaknesses, and recommendation. Text mode only — the web's realtime voice transport is deliberately not mirrored. **All five AI Karier hub tiles now open real screens.**

**Company review authoring, salary submission, and avatar upload are now built.** Reviews: a "Tulis review" button on the company detail opens a `ReviewSheet` (star rating, headline, employment status, pros/cons, recommend/anonymous toggles) posting to `companies/{slug}/reviews`. Salary: a FAB on Insight Gaji opens `SalarySubmitSheet` posting to `salary-submissions` with `sample_size: 1` and a fixed source filled in by the controller. Avatar: `ProfileEditController.pickAvatar` (file_picker, 4 MB / jpg-png-webp) rides in the profile update as multipart — `EmployeeRepository.updateProfile` switches to `FormData` when `avatarPath` is set, and **booleans become `'1'`/`'0'`** there because Laravel's `boolean` rule rejects the strings `'true'`/`'false'`. The picked image previews via `Image.memory` (bytes, not a `dart:io` File, so the web build still compiles) and uploads from the path (mobile only — web has no path and toasts that it is unsupported). The avatar now renders in the Profil header and the edit form; it refreshes there after the user is re-fetched (next session), since `POST profile` returns no new URL to set optimistically.

The **onboarding wizard** still has no avatar step. Unconsumed endpoints: `PUT profile/skills`, `POST job-alerts/{id}/dispatch`, `POST conversations/start`, `GET search`, `GET dashboard`, `reviews/{id}/helpful`, `GET settings`, `faqs` / `about` / `legal`, `POST contact`.
